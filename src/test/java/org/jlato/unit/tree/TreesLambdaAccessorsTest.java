...
public class TreesLambdaAccessorsTest {
	...
	public void testFormalParameter() {
		...
		for (int i = 0; i < 10; i++) {
			...
			NodeList<AnnotationExpr> ellipsisAnnotations2 = arbitrary.arbitraryListAnnotationExpr();
			...
			FormalParameter t2 = t.withModifiers(mutationBy(modifiers, modifiers2)).withType(mutationBy(type, type2)).setVarArgs(mutationBy(isVarArgs, isVarArgs2)).withEllipsisAnnotations(mutationBy(ellipsisAnnotations, ellipsisAnnotations2)).withId(mutationBy(id, id2)).setReceiver(mutationBy(isReceiver, isReceiver2)).withReceiverTypeName(mutationBy(receiverTypeName, receiverTypeName2));
			Assert.assertEquals(modifiers2, t2.modifiers());
			Assert.assertEquals(type2, t2.type());
			Assert.assertEquals(isVarArgs2, t2.isVarArgs());
			Assert.assertEquals(ellipsisAnnotations2, t2.ellipsisAnnotations());
